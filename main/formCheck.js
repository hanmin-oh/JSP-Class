...
//	=> form이라는 id 속성이 지정된 form에서 submit 버튼이 클릭되면 submit() 함수의 인수로 지정한 익명
//	   함수가 실행된다. 이 때, 익명 함수의 인수(event)로 실행되는 이벤트가 넘어온다. 
	$('#form').submit(function (event) {
...
//	실행된다.
//	each() 함수는 익명 함수로 2개의 인수를 넘기는데 첫 번째 인수는 선택된 객체의 인덱스이고 두 번째 인수는
