function tableAdd() {
    ...
    //forms: 현재 문서의 form을 배열 형태로 저장하고 있는
    let form = document.forms[0];
    //console.log(form);

    //form객체가저장된변수.name속성값.value: form에서 지정된 name 속성을 가지는 요소의 입력된
    //값을 얻어올 수 있다.
        ...
        if(value == null || value.trim() == '' || value == undefined) {
            switch (i) {
                case 0:
                    ...
                    form.id.value = '';
                    form.id.focus();
                    //테이블에 추가할 데이터가 모두 입력되지 않았으므로 return으로 tableAdd() 함수를 종료한다.
                    return;
                case 1:
                    alert('비밀번호를 입력해주세요');
                    form.password.value = '';
            ...
            // 테이블에 추가할 데이터가 입력되지 않았으므로 tableAdd() 함수를 종료한다.
            // return; //현재 실행하고 있는 함수가 종료된다.
        }
    }
    // 배열에 저장된 데이터를 id 속성이 table인 테이블에 추가한다.
    //<tbody> 태그를 얻어온다.
    let tbody = document.getElementById('addtr');
    // 배열에 저장된 데이터를 사용해서 <tbody> 태그에 추가할 행(<tr> 태그)을 만드는 함수를 실행한다.


    // <tbody> 태그에 행(함수가 리턴한 결과)을 추가한다.
    tbody.appendChild(creatRow(values));

    /*//let addTable = document.getElementsByTagName('th')[4];
    //addTable.innerHTML = form.password.value; */
    let a = document.getElementById('addtr');
    a.innerHTML = form.id.value;
...
function creatRow(values) {
    ...
    //현재 데이터를 삭제하는 버튼을 추가한다.
    let td = document.createElement('td');
    // td.innerHTML = '<input type="button" value="삭제" onClick=""/>';
    // td.innerHTML = '<input type="button" value="' + values[0] + '님의 데이터 삭제" onClick=""/>';
    td.innerHTML = `<input type="button" value="${values[0]}  님의 데이터 삭제" onClick="removeCurrent(this)"/>`;
    tr.appendChild(td);
