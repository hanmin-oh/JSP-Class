...
<%-- jsp요소는 html 주석으로 주석 설정이 되지 않는다.
 <%@ ~ %> : 디렉티브 => 대부분 자동으로 입력된다. import 같이 설정에 관련된 정보를 적는다.
 <%! ~ %> : 선언부 => 프로그램에서 사용할 변수나 메소드를 정의한다. => java 파일로 뽑아내서 사용한다.(사용 X)
 <%= ~ %> : 표현부 => 변수에 저장된 결과나 연산 결과를 출력 => EL(&{ ~ })로 대체해서 사용한다.
...
<!-- 
	...
	html은 spacebar를 여러번 입력해도 1칸밖에 떨어지지 않고 enter키를 여러번 눌러도 줄이 변경되지 않는다.
	2칸 이상 연속해서 띄우려면 &nbsp;를 띄울 칸에 개수만큼 입력하고 줄을 바꾸려면 <br>태드를 사용한다.
	
 -->

myInfo.jsp&nbsp;&nbsp;&nbsp;입니다.<br>반갑습니다.
...
<form action="myInfoOK.jsp" method="post">
	...
	<fieldset style="width: 120px;">
		...
		<input type="radio" name="gender" value="flase" checked="checked">여자
	</fieldset>
	...
	<!-- 
	 - 콤보상자나 목록상자는 별도의 스타일을 지정하지 않으면 <option>태그에 가장 긴
	문자열 길이에 맞춰서 크기가 결정된다. 
	 - 콤보상자는 첫 번째 <option> 태그에 입력한 내용이 기본적으로 표시되는데 selected
	 ="selected" 속성을 지정하면 이 속성이 지정된 <option> 태그에 입력한 내용을 기본값
	 으로 표시할 수 있다.
	 -->
	...
	<!-- 
	 ...
	 - 목록상자에 표시되는 아이템의 개수는 4개가 기본값이고 size 속성을 지정하면
	 4개가 기본값으로 표시되고 size 속성을 지정하면 지정한 만큼 추가
	 -->
	...
	<!--
	 - <textarea> 태그에는 value 속성이 없고 <textarea>사이를 value로 취급
	...
	<input type="button" value="그냥버튼" onclick="alert('!!!!!!')">
	<button type="submit">정보보기</button>
	<button type="reset">다시쓰기</button>
	<button type="button" onclick="alert('!!!!!!')">그냥버튼</button>
