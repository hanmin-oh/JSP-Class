...
/* - get 방식도 한글이 깨졌었었다. => tomcat server 7.0부터 get방식 한글 안깨짐
 - get 방식에서 한글이 깨지면 서버 환경 설정에서 한글이 깨지지 않도록 설정해야 한다.
 - Servers 폴더의 server.xml 파일을 열고 Connector 태그에 URLEncoding="UEF-8"속성을 
추가하면 된다. 
 - 서버에 추가하기 때문에 프로젝트를 생성할 때 마다 설정할 필요가 없고
이클립스를 다시 세팅할 때 하면 된다. */
...
//이전 페이지에서 넘어오는 데이터는 무조건 문자열 형태로 넘어온다.
	...
	out.println(name + "님의 올해 나이는 " + age + "살 입니다<br>");
	out.println(name + "님의 내년 나이는 " + (age+1) + "살 입니다<br>");
	} catch (Exception e) {
		out.print("<script>");
		out.print("alert('잘못된 나이가 입력되었습니다.')");
		out.print("</script>");
	}
	boolean gender = Boolean.parseBoolean(request.getParameter("gender")); 
	out.println(name + "님은" + (gender ? "남자" : "여자") + "입니다<br>");
	
	//checkbox는 여러 항목을 선택할 수 있는데 아래와 같이 request.getParameter() 메소드를 이용해 넘어오는 데이터를
	//받으면 맨 처음 선택한 값 1개만 받을 수 있다.
