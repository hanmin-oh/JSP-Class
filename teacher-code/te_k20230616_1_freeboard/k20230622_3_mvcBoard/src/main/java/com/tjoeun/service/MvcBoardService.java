...
//	만들어 request 영역에 저장하는 메소드를 호출하는 메소드
	public void selectList(HttpServletRequest request, HttpServletResponse response) {
		...
		// 브라우저에 표시할 페이지 번호가 정상적으로 넘어왔다면 넘어온 페이지 번호로, 정상적으로 넘어오지
...
//	넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조회수를 증가하는 update sql 명령을 실행하는 메소드를
//	호출하는 메소드	
...
//	넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조회수를 증가시킨 글 1건을 얻어오는 select sql 명령을
//	실행하는 메소드를 실행하고 조회수를 증가시킨 글 1건을 request 영역에 저장하는 메소드를
...
//	request 객체를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 글 1건을 수정하는 update sql 명령을
//	실행하는 메소드를 실행하는 메소드
...
//	컨트롤러에 delete.nhn이라는 요청이 들어오면 컨트롤러에서 호출하는 메소드로 삭제할 글번호가 저장된
//	request 객체를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 글 1건을 삭제하는 delete sql 명령을
//	실행하는 메소드를 실행하는 메소드
...
//	request 객체를 넘겨받고 mapper를 얻어온 후 MvcBoardDAO 클래스의 조건에 만족하는 seq를 1씩 증가시키는
