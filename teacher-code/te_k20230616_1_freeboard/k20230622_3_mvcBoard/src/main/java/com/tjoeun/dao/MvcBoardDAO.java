...
//	MvcBoardService 클래스에서 호출되는 mapper를 넘겨받고 테이블에 저장된 전체 글의 개수를 얻어오는
...
//	MvcBoardService 클래스에서 호출되는 mapper와 조회수를 증가시킨 글번호를 넘겨받고 조회수를 증가시킨 글을
...
//	MvcBoardService 클래스에서 호출되는 mapper와 삭제할 글번호를 넘겨받고 글 1건을 삭제하는 mvcboard.xml 
...
//	MvcBoardService 클래스에서 호출되는 mapper와 gup와 seq가 저장된 HashMap 객체를 넘겨받고 조건에 만족하는
//	글의 seq를 1씩 증가시키는 mvcboard.xml 파일의 update sql 명령을 실행하는 메소드
