...
// 브라우저의 주소창에 ContextPath 뒤에 @WebServlet 어노테이션의 인수로 지정된 요청이 들어오면(/k20230621_1_fileDownload뒤에  /DownloadAction이 붙어서)
...
//		파일 형태의 데이터를 전송한다는 것을 사용자에게 알려준다. 
		...
		//파일 이름을 헤더에 저장한다.
		//attachment 뒤의 filename은 반드시 큰따움표를 사용해서 묶어야 한다. => \"를 사용한다. 
		response.setHeader("content-Disposition", "attachment;filename=\"" + downloadName + "\";");
		
		//파일을 전송할 준비를 한다. 
		//다운로드 할 디스크이 파일을 읽어들이는 객체를 선언한다. 
...
//		 - read() 메소드는 파일에서 읽어들인 데이터를 배열의 시작 인덱스 부터 마지막 인덱스 까지 채우고 읽어들인
