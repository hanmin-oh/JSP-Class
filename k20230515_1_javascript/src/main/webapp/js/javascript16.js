...
//	그래서 ES6 이후에 이를 보완하기 위해서 추가된 변수 선언 방식이 let과 const이다.

let id = 'ulji'; //변수 선언
console.log('id: ' + id);

//	let으로 선언한 id라는 변수를 같은 이름으로 다시 선언하면 이미 변수가 선언된 상태이므로
//	Uncaught SyntaxError: Identifier 'id' has already been declared와 같은 메시지가 출력되며
// 	프로그램이 종료된다.
...
//	const test = 'study'; // 에러 발생

//	const로 선언한 변수는 상수이므로 값을 할당하면 상수는 한 번 초기화시키면 프로그램에서 값을 
//	변경시킬 수  없으므로 Uncaught TypeError: Assignment to constant variable. 
//	와 같은 메시지가 출력되며 프로그램이 종료된다. 
//	test = 'check'; //에러발생

//	let과 const는 변수 재선언이 되지 않는다는 공통점이 있고 차이점은 let은 선언된 변수에 데이터 재할당이
//	가능하지만 const는 선언시 할당된 데이터를 프로그램에서 재할당할 수 없다. 
console.log('============================================');
...
function scopeTest() {
	...
	if(true) {
		...
		console.log('for 블록에서 선언한 i를 for 블록 이전에서 출력: ' +i);
		for(var i = 0 ; i<5 ; i++) {
	...
	console.log('scopeTest() 함수 블록에서 선언한 a: ' + a);
	console.log('if 블록에서 선언한 b: ' + b);
	console.log('for 블록에서 선언한 i: ' + i);a
...
function test1() {
	...
	//innerText : 선택된 객체 내부에 html 태그 구현이 불가능한 문자열을 넣거주거나 얻어온다.
...
function test3() {
	...
	document.getElementById('var2').innerHTML = 
		'<i style="color: aqua; background-color : green;">' + variable2 + '</i>';
}
...
function test4() {
	...
	variable3 = [1,2];
	console.log(typeof variable3);	//배열도 object
	variable3 = {name: '이순신'};
