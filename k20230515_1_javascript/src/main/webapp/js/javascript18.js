...
*/
function func3 () { literalTest(msg => alert(msg)) };
...
//	인수 목록에서 가인의 개수만큼 반복하며 arguments 객체에 저장된 데이터를 꺼내서 가인수에 넣어준다.
...
function closureTest() {	// 외부 함수 => 클로저
	var a = 100;
...
const goodEdu = closureTest();

/* closureTest() 함수가 실행된 후 내부 함수를 리턴시키므로 closureTest() 함수가 호출된 곳으로
closureTest() 내부 함수인 addValue() 함수가 리턴되므로 
