...
function sendID2() {
	...
	//id가 아닌 name 속성이나 tag 이름으로 객체를 얻어오려면 id는 무조건 1개만
	//나올 수 있으므로 Element 처럼 단수형을 사용하고 name 속성이나 tag 이름은 
	//같은 것이 여러 개 나올 수 있으므로 Elements를 사용한다.
	//getElementById() 함수는 1개만 얻어오기 때문에 그냥 사용하면 되지만
	//getElementsByName() 함수는 여러개를 배열로 얻어오기 때문에 그냥 사용하면 안되고
	//인덱스를 지정해서 사용해야 한다.
	...
	//getElementsClassTagName() : 인수로 지정된 Class 속성을 가지는 객체들을 얻어온다.
