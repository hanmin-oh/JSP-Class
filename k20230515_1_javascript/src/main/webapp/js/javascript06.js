...
console.log(cat);
// 객체 외부에서 만든 함수를 say() 함수를 cat 객체에 주입한다.
cat.say = say;
console.log(cat);
cat.say();
...
// getter, setter를 실행할 때 함수 뒤에 ()를 붙이면 안된다.
...
const bull = {
	...
	//getter => get으로 선언한다, 특정 멤버의 값이나 연산 결과를 얻어와야 하므로 일반적으로 값을 return 시켜야 한다.
	get name() {
		console.log('_name을 조회한다.')
		return this._name;
	},
	//setter => set으로 선언한다, 특정 멤버의 값이나 연산 결과를 넣어줘야 하므로 인수로 value를 지정해야 한다.
	set name(value) {
