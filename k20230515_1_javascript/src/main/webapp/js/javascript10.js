...
//accumulator의 초기치를 생략하면 배열의 0번째 index의 값이 초기치가 되고 1번째 index부터 반복.
//accumulator의 초기치를 지정하면 배열의 0번째 index부터 반복된다.
*/

// accumulator : 필수 입력, 연산 결과가 저장될 기억장소
// currentValue : 생략 가능, redruce() 함수를 실행하는 배열에 저장된 값이 차례대로 넘어와 저장되는 변수
// currentIndex : 생략 가능, currentValue로 넘어오는 값의 index가 저장되는 변수
// array: reduce() 함수를 실행하는 배열 자신이 넘어와 저장되는 변수
...
function tenOverNumber3(numbers) {
	return numbers.reduce(function (acc, value ) {
		if(value > 10) {
			...
			//acc+을 사용하면 현재 acc값인 0을 다음 실행의 acc에 넣어주기 때문에 최종 결과가
			//0이 나온다.
...
function tenOverNumber4(numbers) {
	return numbers.reduce((acc, value) => (value > 10 )? ++acc : acc ,0);	
}
count = tenOverNumber4(numbers2);
console.log(count);
console.log('=====================================');
...
function alphaCount(alphas) {
	alphas.reduce(function (acc, value) {
