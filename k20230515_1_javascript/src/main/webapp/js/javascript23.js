//객체 선언 => function 사용
function Clazz(name) {
	//객체를 구성하는 멤버 변수를 선언할 때 this를 붙여주지 않으면 객체 외부에서 접근이 불가능하다.
	this.name = name;
...
//json 객체 선언 => key 부분을 반드시 큰따움표로 묶어야 한다.
const Clazz3 = {
	"name": '강감찬',
	"age": 56,
	"getAge": function () {
...
function objectJSON() {
	...
	//json을 가장한 문자열 => key 부분은 반드시 큰따움표를 사용해서 문자열로 만든다.
	let myJson = '{"name": "홍범도", "age": 41}';
	console.log(myJson);
