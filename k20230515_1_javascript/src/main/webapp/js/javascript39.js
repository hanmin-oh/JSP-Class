function addAppend() {
    ...
    p.innerHTML = '<b>fieldset 태그 내부의 div 태그 뒤에 추가.</b> '

    //creatTextNode(): 태그 내부에 넣어줄 문자열을 만든다. => 태그는 적용되지 않는다.
    //appendChild(): 인수로 지정된 태그를 뒤로 추가한다.
    /*let txt = document.createTextNode('<b>fieldset 태그 내부의 div 태그 뒤에 추가.</b>');
    p.appendChild(txt);*/
...
function addBefore() {
    ...
    // let txt = document.createTextNode('<b>fieldset 태그 내부에 div 태그 앞에 추가한다.</b>');
    // p.appendChild(txt);
    //textContent: createTextNode() 함수로 문자열을 만들고 appendChild() 함수를 실행한 것 처럼 태그
    // 내부에 문자열을 넣어준다.
    p.textContent = '<b>fieldset 태그 내부에 div 태그 앞에 추가한다.</b>';
