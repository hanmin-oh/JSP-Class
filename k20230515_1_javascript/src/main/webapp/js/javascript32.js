...
function chkSelect() {
	...
	for(let checkBox of checkBoxs) {
		...
		//빨강, 파랑, 노랑, 검정 체크박스가 모두 체크되었으면 전체선택 체크 박스에 체크하고 한개라도
	...
	for(let checkBox of checkBoxs) { 
		...
		if(!checkBox.checked) {
			flag = false;
			// 각각의 체크 박스 중에서 한개라도 체크가 안되어져있으면 나머지는 비교할 필요가 없다.
			break;
		}
	}
...
function selectColor() {
	...
	for(let checkBox of checkBoxs) { 
		...
		// 선택된 체크 박스의 배경색과 글자색을 원래대로 되돌린다.
...
function clearColor() {
	...
	// 전체 선택 체크박스를 해제하고 모든 체크박스를 해제한다.
