...
function strTest3() {
	let str = prompt('니 이름이 뭐니?' , '숫자는 입력하시 마세요');
	...
	// '==' '!='는 데이터 타입은 비교하지 않고 데이터만 비교한다. '1'과 1을 같은 값으로 비교
...
function strTest4() {
	// indexOf() : 문자열의 왼(앞)쪽 부터 인수로 지정한 문자열을 찾아 index를 리턴한다.
	// lastIndexOf() : 문자열의 오른(뒤)쪽 부터 인수로 지정한 문자열을 찾아 index를 리턴한다.
	// indexOf() , lastIndexOf() 모두 인수로 지정한 문자열이 없으면 -1을 리턴한다.
	let name = '이순신 을지문덕 강감찬 홍범도 이순신';
