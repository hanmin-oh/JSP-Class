...
function confirmChk() {
	...
	if(id.length == 0) {
		...
		변수가 정의되지 않았다는 에러가 발생한다.
		- id라는 변수에 'lee'가 저장되어 있을 때 id라는 변수를 함수로 전달하면 'lee'이라는 문자열로
		인식하는 것이 아니고 lee라는 변수로 인식한다.
		- 변수에 저장된 문자열을 함수의 인수로 전달하려면 변수 이름 앞뒤에 따움표를 붙여서 전달해야
		한다. 
		- 큰 따움표와 작은 따움표가 모두 사용된 상태일 경우 또 따움표를 찍어야 한다면 \" 또는 \'를 
		사용해야 한다. */
	}
	...
	/* if(id.length == 0) {
		div.innerHTML = '<b>중복 검사할 아이디를 입력하세요.</b>';
	} else {
		for(let i = 0 ; i<ids.length ; i++) {
			if(id == ids[i]) {
				div.innerHTML = '<b>사용할 수 없는 아이디입니다.</b>';
			} else {
				div.innerHTML = '<br/><b><span style="color: red;">' + id + 
					'</span> 는(은) 사용할 수 있는 아이디입니다.</b>';
			}	
...
function insertID(id) {
	//console.log(id);
	// 중복 검사를 통과한 아이디를 부모 창으로 넘기고 비밀번호를 포커스에 위치시킨다.
