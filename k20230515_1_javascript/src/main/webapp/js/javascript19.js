function openWin() {
	...
	let url='./20_javascript_window2.html';
	let title = '아이디 중복 검사'; 
...
function closeWin() {
	// id 속성이 register로 지정된 div태그를 화면에서 사라지게 한다.
...
function idCheck() {
	let url='./21_javascript_window3.html';
	let title = '아이디 중복 검사'; 
	let option = 'top=50px, left=2100px, width=500px, height=600px';
	window.open(url, title, option);
}
