...
<%--
    EL을 사용하면 request 영역에 저장된 데이터를 받는 처리 없이 사용할 수 있다.
    ...
    EL을 사용하면 객체 내부의 데이터(필드에 저장된 값)를 ${객체이름, 필드이름}형태로 얻어올 수 있다.
    => getter가 자동으로 실행된다.
     out.println(guestbookList.getList());
     EL을 사용하면 ${guestbookList.list}와 같이 사용할 수 있다.
     ...
     <c:forEach var="변수이름" begin="초기치" end="최종치" [step="증가치(생략하면1)"]>
        반복할 문장
    </c:forEach>
