...
//    request영역에 저장되는 데이터 타입은 Object 타입이므로 반드시 데이터를 얻어온 다음에 형변환 시켜야 한다.
...
<table width="1000" border="1" align="center" cellpadding="5" cellspacing="0">
    ...
    //                ArrayList에 저장된 글의 개수만큼 반복하며 출력한다.
            ...
            <table class="table1" width="99%" border="1" align="center" cellpadding="5" cellspacing="0">
                <tr>
                    <td>
                        <%=vo.getIdx()%>. <%=vo.getName().replace("<" , "&lt;").replace("<", "&lt;").replace("\r\n", "</br>")%>
                        (<%=vo.getIp()%>)님이
                        <%
                        ...
                        <div style="float: right;">
                            <input type="button"
                                    value="수정"
                                    onclick="location.href='selectByIdx.jsp?idx=<%=vo.getIp()%>&currentage=<%=guestbookList.getCurrentPage()%>&job=update'"/>
                            <input type="button"
                                    value="삭제"
                                    onclick="location.href='selectByIdx.jsp?idx=<%=vo.getIp()%>&currentage=<%=guestbookList.getCurrentPage()%>&job=delete'"/>
                        </div>
                        <br/>
