function setting(idx, mode, title, name, content) {
	...
	//fmt.content.value = content;
	//위와 같이 실행해서 처리가 되면 좋겠으나...enter(\r\n)때문에 문제가 발생한다. 
	
	//${co.content}를 인수로 넘기면 자바스크립트는 이스케이프 시퀸스(\r\n)가 포함된 데이터를 인수로 받을
	//수 없기 때문에 정상적으로 동작되지 않는다.
	//${content}느 \r\n이 입력된 부분을 브라우저에 줄을 바꿔 출력하기 위해 <br/> 태글 치환한 데이터가
