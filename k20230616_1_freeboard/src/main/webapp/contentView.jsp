	...
	<table class="table" style="width: 700px; margin-left: auto; margin-right: auto;">
		...
		<tr class="table-secondary">
			<td colspan="4" align="center">
				...
				<!--
					history.back(), history.go(-1)를 사용하면 단순히 전 화면으로 돌아가기 때문에 증가된
					조회수가 반영되지 않은 상태로 브라우저에 보여진다.
				-->
				<input 
   ...
   <tr class="table-success">
      ...
      <tr>
         ...
         <td style="width: 250px;">
            <input id="name" class="form-control-sm" type="text" name="name"/>
         </td> 
         <th class="align-middle" style="width: 100px; text-align: center;">
            <label for="password">비밀번호</label>
         </th>
         <td style="width: 250px;">
            <input id="password" class="form-control-sm" type="password" name="password"/>
         </td> 
      </tr>
      <tr>
         <th class="align-middle" style="width: 100px; text-align: center;">
      ...
      <c:if test="${comment.size() != 0 }">
  	  <c:forEach var="co" items="${comment}">
	  	  <tr>
				...
				${co.name} 님이
				<fmt:formatDate value="${vo.writeDate}" pattern="yyyy.MM.dd(E)"/>에 남긴 글
				...
				<br/>
				<div align="right">
							<!-- 자바스크립트 함수의 인수로 문자열을 전달할 경우 반드시 따움표 묶어야 한다.  -->
						<input 
							class="btn btn-outline-primary btn-sm" 
							type="button" 
							value="수정하기"
