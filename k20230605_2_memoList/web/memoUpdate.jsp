...
<%
    ...
    if(tempIdx != null && tempIdx.trim().length() != 0 && tempCurrentPage != null && tempCurrentPage.trim().length() != 0) {
        //  수정할 글번호 또는 돌아갈 페이지 번호가 모두 숫자인지 검사한다.
        try {
            int idx = Integer.parseInt(tempIdx);
            int currentPage = Integer.parseInt(tempCurrentPage);

            // 3. 수정할 글번호에 해당되는 글이 테이블에 존재하는가 확인하기 위해 글번호에 해당되는 글을 얻어온다.
...
<%-- 수정할 글이 존재하므로 수정할 글을 화면에 보여주고 비밀번호를 입력받는다.--%>
                <form action="updateOk.jsp" method="post">
                    <table width="1000" border="1" align="center" cellpadding="5" cellspacing="0">
                        ...
                        <tr>
                            <td align="center" rowspan="2"><%=rs.getInt("idx")%></td>
                            <td align="center"><%=rs.getString("name").replace("<" , "&lt;") %></td>
                            <td><%=rs.getString("memo").replace("<" , "&lt;") %></td>
                            <td align="center" rowspan="2"><%=sdf.format(rs.getTimestamp("writeDate")) %></td>
                            <td rowspan="2"><%=rs.getString("ip") %></td>
                        </tr>
                        <tr>
                            <td align="center">
...
<%
                ...
                //수정할 글이 테이블에 존재하지 않기 대문에 에러 메시지를 출력하고 memoList4로 돌려보낸다.
                out.println("<script>");
            ...
            //수정할 글번호 또는 돌아갈 페이지 번호가 숫자가 아니므로 에러 메시지를 출력하고 memoList4로 돌려보낸다.
            out.println("<script>");
